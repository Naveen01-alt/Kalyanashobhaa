...
import { 
  ...
  Crown, Sparkles, Phone, Mail, Calendar, Hash
} from 'lucide-react';
...
// Categorized Cities
const FREQUENT_CITIES = ["Hyderabad", "Warangal", "Nizamabad", "Karimnagar", "Khammam", "Visakhapatnam", "Vijayawada", "Guntur", "Tirupati"];
const OTHER_CITIES = ["Bangalore", "Mysore", "Mangalore", "Chennai", "Coimbatore", "Madurai", "Mumbai", "Pune", "Nagpur", "Delhi", "Noida", "Gurgaon", "Kolkata", "Ahmedabad", "Surat", "Jaipur", "Lucknow", "Patna", "Bhopal", "Indore", "Chandigarh", "Kochi", "Thiruvananthapuram"];
...
const INITIAL_FILTERS = {
  ...
  minAge: '', maxAge: '', religion: 'Hindu', community: '', 
...
const AdminUserManagement = () => {
  ...
  // Fetch Master Communities on Mount
  useEffect(() => {
    const fetchCommunities = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/public/get-all-communities`);
        if (response.data.success) {
          setMasterCommunities(response.data.data);
        }
      } catch (err) {
  ...
  const clearAdvanced = () => {
    ...
    setAvailableSubCommunities([]); // Clear sub-communities on reset
          ...
          {showAdvanced ? (
            <form onSubmit={executeAdvancedSearch} className="um-adv-form">
              <div className="um-form-grid">
                ...
                <select name="religion" value={advFilters.religion} onChange={handleAdvChange} className="um-input">
                ...
                <select name="city" value={advFilters.city} onChange={handleAdvChange} className="um-input">
                    ...
                    <optgroup label="Frequently Used">
                      {FREQUENT_CITIES.map(c => <option key={c} value={c}>{c}</option>)}
                    </optgroup>
            ...
            <div className="um-detail-column highlight">
              ...
              <div className="um-contact-box">
                ...
                <div className="um-contact-item">
                    ...
                    <a href={`mailto:${user.email}`} className="value">{user.email}</a>
                </div>
              </div>
            </div>
          </div>
